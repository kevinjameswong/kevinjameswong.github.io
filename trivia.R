    ...
    tabItems(
      ...
      tabItem(tabName = "Analysis",
              ...
              fluidRow(width = 12,
                       ...
                       column(width = 6, tableOutput(outputId = "Table_AvgPoints_Scored_Weighted_Category"))
              ),
...
serverx = function(input, output, session) {
  ...
  output$Table_AvgPoints_Scored_Weighted_Category = renderText({
    trivia_TT() %>%
      filter(Round %in% c("Bonus", "Halftime", "Final", "Tiebreaker") == FALSE) %>%
      group_by(Category) %>%
      summarize(TotalQuestions = n(),
      ...
      kable(format = "html", align = "llll", caption = "Top 10 Best Categories (Average Points, Weighted) (2+ Categories)", col.names = c("Category", "Accuracy", "Total Questions", "Points Scored (Avg, Weighted)")) %>%
      kable_styling(bootstrap_options = c("hover", "responsive", "striped")) %>%
      scroll_box(width = "100%", height = "100%")
  })
  
  output$Table_TotalPoints_Wasted_Weighted = renderText({
    trivia_TT() %>%
      filter(Round %in% c("Bonus", "Halftime", "Final", "Tiebreaker") == FALSE) %>%
      group_by(Category) %>%
      summarize(TotalQuestions = n(),
      ...
      arrange(desc(Points_Wasted_Weighted), desc(TotalQuestions), Category) %>%
      head(n = 10) %>%
      ungroup() %>%
      select(Category, TotalQuestions, Correct, Accuracy, Points_Wasted_Weighted) %>%
      kable(format = "html", align = "lllll", caption = "Top 10 Wasted Categories (Total Points, Weighted)", col.names = c("Category", "Total Questions", "Total Correct", "Accuracy", "Points Wasted (Total, Weighted)")) %>%
      kable_styling(bootstrap_options = c("hover", "responsive", "striped")) %>%
      scroll_box(width = "100%", height = "100%")
  })
