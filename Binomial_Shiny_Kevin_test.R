...
server_a = function(input, output) {
  ...
  output$aye = renderText({
    ...
    results = list(values$trials, values$successes, values$prob, values$prob_less, values$prob_less_or, values$prob_equal, values$prob_greater_or, values$prob_greater) # prob_between)
    names(results) = c(
      "Trials",
      "Successes",
